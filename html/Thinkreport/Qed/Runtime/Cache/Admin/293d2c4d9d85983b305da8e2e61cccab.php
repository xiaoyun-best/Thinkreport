...
<ul class="breadcrumb">当前位置：
  <a href="#">科室管理</a> <span class="divider">
  <span class="pull-right margin-bottom-5 gohistory" style='cursor:pointer;color:#000;'>返回上一级</span> 
</ul>
<div class="title_right"><span class="pull-right margin-bottom-5"><a  class="btn btn-info btn-small"  id="modal-9735581" href="<?php echo U('Tracer/editTracer',array('menuname'=>'添加科室'));?>" role="button" ><i class="icon-plus icon-white"></i>添加科室</a></span><strong>科室列表</strong></div>
<form method="get" name='form'>
      <table style="width:100%;">
        <tr align="left">
        <td  align="left" nowrap="nowrap" bgcolor="#f1f1f1" >
        科室：<input type="search" name='tbldepartment_department'  value="<?php echo ($_GET['tbldepartment_department']); ?>" />
...
<table class="table table-bordered table-striped table-hover">
     <tbody>
      ...
      <?php if($list): if(is_array($list)): foreach($list as $key=>$val): ?><tr align="center" class='itemChange'>
         <td nowrap="nowrap"><?php echo ($val['tbldepartment_id']); ?></td>
         <td nowrap="nowrap"><?php echo ($val['tbldepartment_department']); ?></td>
         <td nowrap="nowrap"><input type="hidden" class='id' value="<?php echo ($val["tbldepartment_id"]); ?>" /><a href="<?php echo U('Depart/editDepart',array('menuname'=>'修改科室','id'=>$val['tbldepartment_id']));?>">【修改】</a>&nbsp;&nbsp;<a class='delete' style="cursor:pointer;">【删除】</a>
...
$('.delete').on('click',function(){
  if(confirm('确定要删除吗？')){
    ...
    window.location.href='/Admin/Depart/del/id/'+id;
  }
})
</script>
