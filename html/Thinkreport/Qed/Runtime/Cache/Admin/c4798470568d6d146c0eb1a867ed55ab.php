...
<div class="right"  id="mainFrame">
    <div class="right_cont">
        ...
        <div class="title_right"><span class="pull-right margin-bottom-5"><a  class="btn btn-info btn-small"  id="modal-9735581" href="<?php echo U('Custom/editCustom',array('menuname'=>'添加自定义字段'));?>" role="button" ><i class="icon-plus icon-white"></i>添加自定义字段</a></span><strong>自定义字段列表</strong></div>

        <form method="get" name='form'>
            <table style="width:100%;">
                <tr align="left">
                    <td  align="left" nowrap="nowrap" bgcolor="#f1f1f1" >
                        自定义字段：<input type="search" name='tblcustom_name' value="<?php echo ($_GET['tblcustom_name']); ?>" />
                        显示名：<input type="search" name='tblcustom_displayname' value="<?php echo ($_GET['tblcustom_displayname']); ?>" />
        ...
        <table class="table table-bordered table-striped table-hover">
            <tbody>
            <tr align="center">
                <td nowrap="nowrap sort"><strong>ID</strong></td>
                <td nowrap="nowrap"><strong>自定义字段</strong></td>
                <td nowrap="nowrap"><strong>显示名</strong></td>
            ...
            <?php if($list): if(is_array($list)): foreach($list as $key=>$val): ?><tr align="center" class='itemChange'>
                        <td nowrap="nowrap"><?php echo ($val['tblcustom_id']); ?></td>
                        <td nowrap="nowrap"><?php echo ($val['tblcustom_name']); ?></td>
                        <td nowrap="nowrap"><?php echo ($val['tblcustom_displayname']); ?></td>
                        <td nowrap="nowrap"><input type="hidden" class='id' value="<?php echo ($val["tblcustom_id"]); ?>" />
                            <?php if($val['tblcustom_isshow']==0): ?><a href="<?php echo U('Custom/isshow',array('tblcustom_isshow'=>$val['tblcustom_isshow'],'id'=>$val['tblcustom_id'],'tblcustom_displayname'=>$val['tblcustom_displayname']));?>">【显示】</a>
                                <?php else: ?>
                                <a href="<?php echo U('Custom/isshow',array('tblcustom_isshow'=>$val['tblcustom_isshow'],'id'=>$val['tblcustom_id'],'tblcustom_displayname'=>$val['tblcustom_displayname']));?>">【不显示】</a><?php endif; ?>

                            <a href="<?php echo U('Custom/editCustom',array('menuname'=>'修改自定义字段','id'=>$val['tblcustom_id']));?>">【修改】</a>&nbsp;
...
<script type="text/javascript">
    ...
    $('.delete').on('click',function(){
        if(confirm('确定要删除吗？')){
            ...
            window.location.href='/Admin/Custom/del/id/'+id;
        }
    })
</script>
