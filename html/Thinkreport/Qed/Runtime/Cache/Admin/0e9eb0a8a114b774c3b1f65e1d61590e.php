...
<div class="title_right"><span class="pull-right margin-bottom-5"><a  class="btn btn-info btn-small"  id="modal-9735581" href="<?php echo U('Item/editItem',array('menuname'=>'添加检查项目'));?>" role="button" ><i class="icon-plus icon-white"></i>添加检查项目</a></span><strong>检查项目列表</strong></div>
<form method="get" name='form'>
      <table style="width:100%;">
        <tr align="left">
        <td  align="left" nowrap="nowrap" bgcolor="#f1f1f1" >
        检查项目：<input type="search" name='tblInspectionItem_item' value="<?php echo ($_GET['tblInspectionItem_item']); ?>" />
...
<table class="table table-bordered table-striped table-hover">
     <tbody>
      ...
      <?php if($list): if(is_array($list)): foreach($list as $key=>$val): ?><tr align="center" class='itemChange'>
         <td nowrap="nowrap"><?php echo ($val['tblinspectionitem_id']); ?></td>
         <td nowrap="nowrap"><?php echo ($val['tblinspectionitem_item']); ?></td>
         <td nowrap="nowrap"><input type="hidden" class='id' value="<?php echo ($val["tblinspectionitem_id"]); ?>" /><a href="<?php echo U('Item/editItem',array('menuname'=>'修改检查项目','id'=>$val['tblinspectionitem_id']));?>">【修改】</a>&nbsp;&nbsp;<a class='delete' style="cursor:pointer;">【删除】</a>
...
$('.delete').on('click',function(){
  if(confirm('确定要删除吗？')){
    ...
    window.location.href='/Admin/Item/del/id/'+id;
  }
})
</script>
