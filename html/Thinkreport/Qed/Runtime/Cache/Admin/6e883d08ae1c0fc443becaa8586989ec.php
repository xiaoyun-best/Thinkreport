...
<div class="title_right"><strong>随访信息：
    </tr></strong>
</div>
    <form method="post" action="/Admin/Flup/flupedit" enctype="multipart/form-data">
        <table>
            <tbody>
            <input type="hidden" name="id" value="<?php echo ($vol["tblflup_id"]); ?>" />
            <input type="hidden" name="tblpatient_ID" value="<?php echo ($tblpatient_ID); ?>" />
            <input type="hidden" name="tblstudy_ID" value="<?php echo ($tblstudy_ID); ?>" />
            <tr>
                <td align="right">随访状态</td>
                <td align="left">
                    <input name="tblflup_state" type="radio" value="1" <?php if($vol['tblflup_state']==1): ?>checked="checked"<?php endif; ?> >随访中
                    <input name="tblflup_state" type="radio" value="2" <?php if($vol['tblflup_state']==2): ?>checked="checked"<?php endif; ?> >随访完
                </td>
            </tr>
            <tr>
                <td align="right">随访时间</td>
                <td align="left"><input name="tblflup_date" value="<?php echo ($vol['tblflup_date']); ?>" class="Wdate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" type="text"></td>
            </tr>
            </tr>
            <tr>
                <td align="right">随访结果:</td>
                <td align="left"><textarea name="tblflup_Des" cols="60" rows="35" style="width:200px; height:100px;"><?php echo ($vol['tblflup_des']); ?></textarea></td>
            </tr>
            </tbody>
        </table>
        <div class="page_btm">
