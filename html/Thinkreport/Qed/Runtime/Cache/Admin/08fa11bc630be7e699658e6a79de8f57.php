...
<div class="title_right"><strong><?php echo ($menuname); ?>：
    </tr></strong></div>
<div style="width:900px;margin:auto;">
    <form method="post" action="/Admin/Menu/editMenu" enctype="multipart/form-data">
        <table>
            <tbody>
            <input type="hidden"name="id" value="<?php echo ($val["tblmenu_id"]); ?>" />
            <tr>
                <td align="right">上级菜单：</td>
                <td align="left">
                    <select style="padding:0;" name='tblmenu_PID'>
                        <option  value='0' selected="">顶级菜单</option>
                        <?php if(is_array($menu)): foreach($menu as $key=>$value): if($pid == $value['tblmenu_id']){ ?>
                            <option value="<?php echo ($value['tblmenu_id']); ?>" selected="selected"><?php echo ($value['tblmenu_name']); ?></option>
                            <?php }else{ ?>
                            <option value="<?php echo ($value['tblmenu_id']); ?>"><?php echo ($value['tblmenu_name']); ?></option>
                            <?php } ?>
                            <!--<option value="<?php echo ($value['tblmenu_id']); ?>"><?php echo ($value['tblmenu_name']); ?></option>--><?php endforeach; endif; ?>
                    </select>
                </td>
            </tr>
            <tr>
                <td align="right">菜单名称:</td>
                <td align="left"><input name="tblmenu_name" type="text" value="<?php echo ($val['tblmenu_name']); ?>"/></td>
            </tr>
            <tr>
                <td align="right">菜单地址:</td>
                <td align="left"><input name="tblmenu_url" type="text" value="<?php echo ($val['tblmenu_url']); ?>"/></td>
            </tr>
            </tbody>
        </table>
        <div class="page_btm">
