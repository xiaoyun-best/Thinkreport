...
<ul class="breadcrumb">当前位置：
  <a href="#">病区管理</a> <span class="divider">
  <span class="pull-right margin-bottom-5 gohistory" style='cursor:pointer;color:#000;'>返回上一级</span> 
</ul>
<div class="title_right"><span class="pull-right margin-bottom-5"><a  class="btn btn-info btn-small"  id="modal-9735581" href="<?php echo U('Area/editArea',array('menuname'=>'添加病区'));?>" role="button" ><i class="icon-plus icon-white"></i>添加病区</a></span><strong>病区列表</strong></div>
<form method="get" name='form'>
      <table style="width:100%;">
        <tr align="left">
        <td  align="left" nowrap="nowrap" bgcolor="#f1f1f1" >
       病区：<input type="search" name='tblInpatientArea_area' value="<?php echo ($_GET['tblInpatientArea_area']); ?>" />
...
<table class="table table-bordered table-striped table-hover">
     <tbody>
      ...
      <?php if($list): if(is_array($list)): foreach($list as $key=>$val): ?><tr align="center" class='itemChange'>
         <td nowrap="nowrap"><?php echo ($val['tblinpatientarea_id']); ?></td>
         <td nowrap="nowrap"><?php echo ($val['tblinpatientarea_area']); ?></td>
         <td nowrap="nowrap"><input type="hidden" class='id' value="<?php echo ($val["tblinpatientarea_id"]); ?>" /><a href="<?php echo U('Area/editArea',array('menuname'=>'修改病区','id'=>$val['tblinpatientarea_id']));?>">【修改】</a>&nbsp;&nbsp;<a class='delete' style="cursor:pointer;">【删除】</a>
...
$('.delete').on('click',function(){
  if(confirm('确定要删除吗？')){
    ...
    window.location.href='/Admin/Area/del/id/'+id;
  }
})
</script>
