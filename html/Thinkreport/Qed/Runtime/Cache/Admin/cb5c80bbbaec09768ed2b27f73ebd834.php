...
<div class="title_right"><span class="pull-right margin-bottom-5"><a  class="btn btn-info btn-small"  id="modal-9735581" href="<?php echo U('User/editUser',array('menuname'=>'添加用户'));?>" role="button" ><i class="icon-plus icon-white"></i>添加用户</a></span><strong>用户列表</strong></div>
<form method="get" name='form'>
      <table style="width:100%;">
        <tr align="left">
        <td  align="left" nowrap="nowrap" bgcolor="#f1f1f1" >
        <!-- 商户类型：<select style="padding:0;" name='type'>
           <?php if(is_array($classes)): foreach($classes as $key=>$value): if($_GET['type']==$key){ ?>
             <option align='center' value='<?php echo ($key); ?>' selected=""><?php echo ($value); ?></option>
           <?php }else{ ?>
             <option align='center' value='<?php echo ($key); ?>'><?php echo ($value); ?></option>
           <?php } endforeach; endif; ?>
        </select> -->
        用户名：<input type="search" name='tbluser_loginname'  value="<?php echo ($_GET['tbluser_loginname']); ?>" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        显示名：<input type="search" name='tbluser_displayname'  value="<?php echo ($_GET['tbluser_displayname']); ?>" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<table class="table table-bordered table-striped table-hover">
     <tbody>
       <tr align="center">
           <td nowrap="nowrap sort" style="width:25%;"><strong>登录名</strong></td>
           <td nowrap="nowrap" style="width:25%;"><strong>显示名</strong></td>
           <td nowrap="nowrap" style="width:25%;"><strong>角色</strong></td>
      ...
      <?php if($list): if(is_array($list)): foreach($list as $key=>$val): ?><tr align="center" class='itemChange'>
           <td nowrap="nowrap"><?php echo ($val['tbluser_loginname']); ?></td>
           <td nowrap="nowrap"><?php echo ($val['tbluser_displayname']); ?></td>
           <td nowrap="nowrap">
               <?php if($val['tbluser_role']==null): ?><span style="color:red">暂无角色</span>
                   <?php else: echo ($val['tbluser_role']); endif; echo ($val['tbluser_role']); ?>
           </td>
           <td nowrap="nowrap"><input type="hidden" class='id' value="<?php echo ($val["tbluser_id"]); ?>" />
             <a href="<?php echo U('User/editRole',array('id'=>$val['tbluser_id']));?>">【设置角色】</a>
             <a href="<?php echo U('User/editUser',array('menuname'=>'修改用户','id'=>$val['tbluser_id']));?>">【修改】</a>&nbsp;&nbsp;
